...
function getTime(date) {
  const addZeroToTime = (time) => String(time).padStart(2, '0');
  return `${addZeroToTime(date.getHours())}:${addZeroToTime(date.getMinutes())}:${addZeroToTime(date.getSeconds())}`;
}
...
function getNextFriday(date) {
  const fridayDay = (5 - date.getUTCDay() + 7) % 7 || 7;
  date.setUTCDate(date.getUTCDate() + fridayDay);
  return date;
}
...
function getCountDaysOnPeriod(dateStart, dateEnd) {
  ...
  const dayDiff = timeDif / (1000 * 60 * 60 * 24);
  return dayDiff + 1;
}
...
function formatDate(date) {
  return new Date(date).toLocaleString('en-RU', { timeZone: 'UTC' });
}
...
function getCountWeekendsInMonth(month, year) {
  let count = 0;
  const days = new Date(year, month, 0, 0).getDate();
...
function getWeekNumberByDate(date) {
  ...
  const pastDaysOfYear = (d - yearStart) / (1000 * 60 * 60 * 24);
